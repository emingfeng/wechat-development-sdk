...
class Tools
{
    ...
    public static function upload($url, $file , $type, $form = [])
    {
        return self::doUpload($url, $file, $type, $form);
    }

    public static function doUpload($url,$file, $type, $form)
    {
        $file_info = array(
            'filename' => realpath($file),
            'content-type' =>  self::getExtMine(pathinfo($file, PATHINFO_EXTENSION)),
            'filelength' => filesize($file)+198
        );

        $data = [
            $type => new \CURLFile(realpath($file),'', basename(realpath($file))),
            'form-data' => $file_info
        ];
        ...
        curl_setopt($curl, CURLOPT_HTTPHEADER,['Transfer-Encoding:','Content-Length:'.(filesize($file)+198)]);
        list($content) = [curl_exec($curl), curl_getinfo($curl), curl_close($curl)];
